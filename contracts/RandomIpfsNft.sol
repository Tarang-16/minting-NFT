...
contract RandomIpfsNft is VRFConsumerBaseV2, ERC721URIStorage, Ownable {
    // we will trigger chainlink VRF to get us a random number
    ...
    error RandomIpfsNft_NotEnoughETHSent();
    ...
    // VRF Helpers
    mapping (uint256 => address) public s_requestIdToSender; // we had to create a mapping between requestId and people calling it. Because if we directly use msg.sender, requestID will be assigned to the chainlink node generating randomword.
    ...
    function requestNft() public payable returns(uint256 requestId){
        if(msg.value < i_mintFee) {
            revert RandomIpfsNft_NotEnoughETHSent();
        }
